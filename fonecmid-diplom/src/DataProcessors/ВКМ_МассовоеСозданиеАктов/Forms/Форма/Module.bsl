...
Процедура Заполнить(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Период) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю("Заполните период создания актов реализации товаров и услуг. Обработка не запущена",,"Объект.Период");
		Возврат;
	...
	Если КонецМесяца(Объект.Период.ДатаНачала) <> КонецМесяца(Объект.Период.ДатаОкончания) Тогда 
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю("Период создания актов реализации товаров и услуг должен составлять один полный месяц. Обработка не запущена",,"Объект.Период");
		Возврат;
...
Функция СоздатьДокументыНаСервере()
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(
		ПараметрыВыполнения,
		"Обработки.ВКМ_МассовоеСозданиеАктов.СоздатьСписокДокументовРеализацииТоваровИУслуг",
...
Процедура ПолучитьСписокДокументов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Тогда  // Пользователь отменил задание.
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю("Обработка отменена пользователем. Возможно некоторые документы не созданы. Рекомендуется перезапустить обработку и дождаться ее завершения.");
		Возврат;
	...
	ТекстСообщения = СтрШаблон("Для указанного периода количество успешно созданных документов - %1. Количество не созданных документов по причине ошибок - %2. ", 	
